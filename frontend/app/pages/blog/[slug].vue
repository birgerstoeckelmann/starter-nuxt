...
const hero = computed(
  () => currentPost.value?.image && currentPost.value?.image.length > 0,
);
...
const hasNextPrev = computed(
  () => hasPost.value && (currentPost.value?.prev || currentPost.value?.next),
);
...
<template>
  <div :key="previewTimestamp">
    <div v-if="pending" class="container mx-auto py-12 px-2">Loading...</div>
    ...
    <template v-else-if="hasPost">
      <header
        class="container mx-auto pt-12 pb-6 px-2 text-2xl relative"
        :class="hero ? 'aspect-video' : ''"
      >
        <figure v-if="hero" class="absolute top-0 left-0 w-full h-full">
          <img
            :src="currentPost.image[0].url"
            :alt="currentPost.image[0].alt"
          />
        </figure>
        ...
        <div
          ...
          :class="
            hero
              ? 'text-white bg-black/80 p-4 sm:bottom-0 relative sm:ml-4 sm:max-w-screen-lg sm:absolute sm:rounded'
              : ''
          "
        >
          ...
          <p v-if="currentPost.pageSubheading" class="mt-4">
            {{ currentPost.pageSubheading }}
          </p>
          <div class="text-xs mt-4">
            <p>
              ...
              <template
                v-if="currentPost.category?.length && currentPost.postDate"
              >
                |
              </template>
              <time
                v-if="currentPost.postDate"
                :datetime="currentPost.postDate"
              >
    ...
    <div v-else class="container mx-auto py-12 px-2">Post not found</div>
